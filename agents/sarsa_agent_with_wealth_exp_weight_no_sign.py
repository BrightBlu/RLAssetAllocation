...
class Agent:
    ...
    def train(self, env: MarketEnvironment, n_episodes: int) -> Dict[str, List]:
        for episode in tqdm(range(n_episodes)):
            ...
            while not done:
                # --- 1) Take action and observe next state
                next_state, reward, done, info = env.step(action)
                episode_rewards.append(reward)
                episode_wealth.append(next_state[0])
                episode_actions.append(action)

                # --- 2) Select next action using epsilon-greedy (SARSA is on-policy)
                next_action = self.select_action(next_state)
                next_discrete_action = self._xt_to_action(next_action)
                ...
                # On-policy target: r + gamma * Q(next_state, next_action)
                if done:
                    target = reward
                else:
                    target = reward + self.gamma * self.q_table[next_state_key][next_discrete_action]
