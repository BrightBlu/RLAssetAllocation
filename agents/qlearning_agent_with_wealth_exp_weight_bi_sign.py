...
class Agent:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.all_actions = [(sign, log_value) for sign in [0, 1] for log_value in range(self.action_lower_bound, self.action_upper_bound + 1)]
    ...
    def _action_to_xt(self, action: Tuple[int, int]) -> float:
        """Convert discrete action (sign, log_value) to continuous portfolio weight.
        
        Args:
            action: Tuple of (sign, log_value) where sign is 0 or 1 and log_value is in [-6, 3]
    ...
    def _xt_to_action(self, xt: float) -> Tuple[int, int]:
        ...
        Returns:
            Tuple of (sign, log_value) where sign is 0 or 1 and log_value is in [-6, 3]
        """
        if xt >= 0:
            sign = 1
            log_value = np.clip(round(np.log(xt)), self.action_lower_bound, self.action_upper_bound)
        else:
            sign = 0
            log_value = np.clip(round(np.log(abs(xt))), self.action_lower_bound, self.action_upper_bound)
    ...
    def _init_state_if_needed(self, state_key: Tuple[Tuple[int,int],int]):
        ...
        if state_key not in self.q_table:
            ...
            random_act = self.all_actions[np.random.randint(len(self.all_actions))]
    ...
    def select_action(self, state: np.ndarray) -> float:
        ...
        if np.random.rand() < self.epsilon:
            ...
            discrete_action = self.all_actions[np.random.randint(len(self.all_actions))]
        else:
