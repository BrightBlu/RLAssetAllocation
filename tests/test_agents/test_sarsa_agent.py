...
def test_q_value_update(agent, env):
    ...
    next_action = 1  # SARSA uses next action instead of max Q-value
    ...
    if not done:
        next_q = agent.q_table[next_discrete_state][next_action]
        agent.q_table[discrete_state][action] = old_q_value + agent.learning_rate * (reward + agent.gamma * next_q - old_q_value)
    else:
    ...
    if not done:
        next_q = agent.q_table[next_discrete_state][next_action]
        expected_q = old_q_value + agent.learning_rate * (reward + agent.gamma * next_q - old_q_value)
    else:
